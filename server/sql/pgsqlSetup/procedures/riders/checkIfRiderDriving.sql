...
BEGIN
    IF exists (select * from Deliveries NATURAL JOIN Orders where Deliveries.usr_id = $1 And Orders.status = 'in progress') THEN
        RAISE EXCEPTION '%', 'Rider already has a delivery in progress';
    ELSE
        UPDATE  Deliveries SET Deliveries.usr_id = $1, dr_leave_for_res = Now() WHERE Deliveries.order_id = $2;
