...
riders.queries = {
    ...
    get_recent_deliveries: 'SELECT Orders.usr_id, Orders.order_id, Orders.res_id, Orders.total, Orders.destination_address, Orders.postal_code AS Opc, Deliveries.place_order_time, Restaurants.address,Restaurants.rname, Restaurants.postal_code AS RPC FROM Orders INNER JOIN Restaurants ON Restaurants.res_id = Orders.res_id INNER JOIN Deliveries ON Deliveries.order_id = Orders.order_id WHERE Deliveries.usr_id = $1 AND Orders.status = $2 ORDER BY Deliveries.place_order_time DESC',
